...
function Import-Excel {
  ...
  if($NoHeaders) { 
	for($c=1;$c-le$columns.Count;$c++) { $headers += "Column$c" } }  # If the Excel file has no headers, use Column1, Column2, etc...
  else {
	$headers = $rows | Where-Object { $_.Row -eq 1 } | %{ $_.Value2 }
    for($i=0; $i -lt $headers.Count; $i++) {    # If a column is missing a header, then create one
		if(!$headers[$i]) { 
